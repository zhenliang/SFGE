...
int LoadFont( lua_State* L )
{
  ...
  if (paraNum == 2)
    result = FontManager::GetInstance().LoadFont(path, key);
  else if (paraNum == 3)
  {
    int charSize = lua_tointeger(L, 3);
    result = FontManager::GetInstance().LoadFont(path, key, charSize);
  }
  else if (paraNum == 4)
  {
    int charSize = lua_tointeger(L, 3);
    ...
    result = FontManager::GetInstance().LoadFont(path, key, charSize, charset);
  }
